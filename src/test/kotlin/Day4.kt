...
// TODO very dirty solution, need refactor

private fun String.isValidPassport(): Boolean {
    val passport = this.replace("\n", " ")
    val validColors = listOf("amb", "blu", "brn", "gry", "grn", "hzl", "oth")

    val keys = passport.split(" ").map {
        it.split(":").first()
    }

    val values = passport.split(" ").map {
        it.split(":")[1]
    }
...
// cid (Country ID) - ignored, missing or not.
    ...
    entries.forEach {
        when (it.first) {
            "byr" -> byr = it.second.toInt() in 1920..2002
            "iyr" -> iyr = it.second.toInt() in 2010..2020
            "eyr" -> eyr = it.second.toInt() in 2020..2030
            "hgt" -> hgt = when {
                it.second.contains("cm") -> it.second.replace("cm", "").toInt() in 150..193
                it.second.contains("in") -> it.second.replace("in", "").toInt() in 59..76
                else -> false
            }
            "hcl" -> hcl = it.second.startsWith("#") && it.second.length == 7
            "ecl" -> ecl = it.second in validColors
            "pid" -> pid = it.second.length == 9 && it.second.matches("-?\\d+(\\.\\d+)?".toRegex())
            else -> false
        }
    }
...
class Day4 {
    ...
    @Test
    fun `assert split each password`() {
        val passports = exampleInput.split("\n\n")
        assertEquals("ecl:gry pid:860033327 eyr:2020 hcl:#fffffd\nbyr:1937 iyr:2017 cid:147 hgt:183cm", passports.first())
        assertEquals("iyr:2013 ecl:amb cid:350 eyr:2023 pid:028048884\nhcl:#cfa07d byr:1929", passports[1])
        assertEquals("hcl:#ae17e1 iyr:2013\neyr:2024\necl:brn pid:760753108 byr:1931\nhgt:179cm", passports[2])
    }

    @Test
    fun `assert valid passport`() {
        val passports = exampleInput.split("\n\n")
        assert(passports[0].isValidPassport())
        assert(passports[2].isValidPassport())
    }

    @Test
    fun `assert invalid passport`() {
        val passports = exampleInput.split("\n\n")
        assertFalse(passports[1].isValidPassport())
        assertFalse(passports[3].isValidPassport())
    }
    ...
    inner class PartA {
        ...
        inner class `Example data` {
            @Test
            fun `count valid passports`() {
                val passports = exampleInput.split("\n\n")
                val sum = passports.count { it.isValidPassport() }
                assertEquals(2, sum)
            }
        }
    ...
    inner class PartB {
        ...
        inner class `Example data` {
            @Test
            fun `assert all invalid passports`() {
                val passports = """eyr:1972 cid:100
...
pid:3556412378 byr:2007""".split("\n\n")
        ...
        @Test
        fun `assert all valid passports`() {
            val passports = """pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
