...
fun String.asGroups(): List<String> = this.split("\n\n")

private fun countYesAnswers(group: String): Int {
    return group.replace("\n", "").toList()
        .distinct()
        .count()
}

// TODO this is very ugly, need to find better solution :(
private fun countSameYesInGroup(group: String): Int {
    var sameAnswerInGroup = "abcdefghijklmnopqrstuvwxyz".toSet()
    group.split("\n")
        .map { it.toList() }
        .forEach { person ->
            sameAnswerInGroup = sameAnswerInGroup.intersect(person.toList())
        }
    return sameAnswerInGroup.count()
}

class Day6 {

    @Test
    fun `assert split as groups`() {
        val groups = exampleInput.asGroups()
        assert(groups.size == 5)
        assertEquals("abc", groups.first())
        assertEquals("a\nb\nc", groups[1])
        assertEquals("ab\nac", groups[2])
        assertEquals("a\na\na\na", groups[3])
        assertEquals("b", groups[4])
    }

    @Test
    fun `assert count YES answer in a group`() {
        val groups = exampleInput.asGroups()
        assertEquals(3, countYesAnswers(groups[0]))
        assertEquals(3, countYesAnswers(groups[1]))
        assertEquals(3, countYesAnswers(groups[2]))
        assertEquals(1, countYesAnswers(groups[3]))
        assertEquals(1, countYesAnswers(groups[4]))
    }
    ...
    inner class PartA {
        ...
        inner class `Example data` {
            @Test
            fun `assert sum of unique Yes answers is 11`() {
                val yesCount = exampleInput.asGroups()
                    .sumBy { group -> countYesAnswers(group) }
        ...
        inner class `Real data` {
            @Test
            fun `sum of unique Yes answers`() {
                val yesCount = exampleInput.asGroups()
                    .sumBy { group -> countYesAnswers(group) }
                println("Part A: $yesCount")
            }
        }
    }
